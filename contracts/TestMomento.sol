...
contract TestMomento is IERC20Metadata, Ownable {
    struct User {
        uint256 buy;
        uint256 sell;
    }

    address public marketingAddress = 0xCfc5835d709A837d7445C0a881c293fF58309d5a;
    address public teamAddress = 0x857ed7E7b4C40F29CB7391FCF88EAc76BD284032;
    ...
    modifier lockTheSwap {
        ...
        console.log("lockTheSwap begin: is in swap ->", inSwapAndLiquify);
        _;
        inSwapAndLiquify = false;
        console.log("lockTheSwap end: is in swap ->", inSwapAndLiquify);
    }
    ...
    function setMarketingAddress(address _markeingAddress) public onlyOwner {
        console.log("setMarketingAddress");
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if (_isExcluded[account]) {
            balance = _tOwned[account];
        } else {
            balance = tokenFromReflection(_rOwned[account]);
        }
    ...
    function allowance(address owner, address spender) public view override returns (uint256) {
        console.log("allowance(owner %s, spender %s)", owner, spender);
        return _allowances[owner][spender];
    }

    function approve(address spender, uint256 amount) public override returns (bool) {
        console.log("approve(spender %s, amount %d)", spender, amount);
    ...
    function isExcludedFromReward(address account) public view returns (bool) {
        console.log("isExcludedFromReward(%s)", account);
        return _isExcluded[account];
    }

    function totalFees() public view returns (uint256) {
        console.log("totalFees -> %d", _tFeeTotal);
        return _tFeeTotal;
    }
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        console.log("reflectionFromToken");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        console.log("tokenFromReflection ->", rAmount);
        require(rAmount <= _rTotal, "Amount must be less than total reflections");
        uint256 currentRate = _getRate();
        console.log("tokenFromReflection Result", rAmount / currentRate);
        return rAmount / currentRate;
    }

    function excludeFromReward(address account) public onlyOwner() {
        console.log("excludeFromReward(%s)", account);
    ...
    function includeInReward(address account) external onlyOwner() {
        console.log("includeInReward(%s)", account);
        require(_isExcluded[account], "Account is already excluded");
    ...
    function _getRate() private view returns(uint256) {
        console.log("_getRate");
        (uint256 rSupply, uint256 tSupply) = _getCurrentSupply();
        return rSupply / tSupply;
    }

    function _getCurrentSupply() private view returns(uint256, uint256) {
        console.log("_getCurrentSupply");
    ...
    function isUniswapV2PairAddress(address account) public view returns(bool) {
        console.log("isUniswapV2PairAddress");
        return _isUniswapV2Pair[account];
    }

    function _approve(address owner, address spender, uint256 amount) private {
        console.log("_approve(owner %s, spender %s, amount %d)", owner, spender, amount);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        console.log("_transfer(from %s, to %s, amount %d)", from, to, amount);
        ...
        console.log("End of _transfer");
    }

    function _buyBackAndBurn(uint256 amount) private lockTheSwap {
        console.log("_buyBackAndBurn(amount %d)", amount);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        console.log("swapAndLiquify(contractTokenBalance %d)", contractTokenBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        console.log("swapTokensForEth(tokenAmount %d)", tokenAmount);
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        ...
        console.log("End of swapTokensForEth");
    }

    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        console.log("addLiquidity(tokenAmount %d, ethAmount %d)", tokenAmount, ethAmount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        console.log("_tokenTransfer(sender %s, recipient %s, amount %d)", sender, recipient, tAmount);
        console.log("_tokenTransfer takeFee ->", takeFee);
        if(!takeFee) {
            removeAllFee();
        }
        ...
        console.log("End of _tokenTransfer");
    }
}